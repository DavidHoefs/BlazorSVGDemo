...
{
    private static readonly Dictionary<char, int> _length = new()
    {
        { 'a', 7 },
        { 'c', 6 },
        { 'h', 1 },
        ...
        foreach (var ee in test)
        {
            ...
            foreach (var ee2 in test2)
            {
                ...
                IEnumerable<XElement> test3 = from ee3 in ee2.Elements("{https://www.w3.org/2000/svg}path")
                    select ee3;
                foreach (var epath in test3) sb.AppendLine("     sPath  :" + epath.Attribute("d").Value);
            }
        }
    ...
    public static List<(char, List<double>)> Parse(string path)
    {
        ...
        var segments = Regex.Matches(path, "([astvzqmhlc])([^astvzqmhlc]*)",
        ...
        foreach (Match match in segments)
        {
            ...
            while (args.Count >= 0)
            {
                ...
                if (args.Count < _length[type])
                    throw new Exception(
                        $"Malformed path data: \"{command}\" must have {_length[type]} elements and has {args.Count}: {match.Value}");
    ...
    {
        var numbers = Regex.Matches(args, @"-?[0-9]*\.?[0-9]+(?:e[-+]?\d+)?",
            RegexOptions.IgnoreCase | RegexOptions.Compiled);
